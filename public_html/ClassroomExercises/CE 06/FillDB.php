...
<body>
    ...
    foreach ($planets as $Newplanets) {
        $insert = "INSERT INTO `u743148202_csis_2440`.`PLANETS` (`PLANET_NAME`, `COORD_X`, `COORD_Y`, `COORD_Z`, `DESCRIPTION`) "
    ...
    for ($i = 0; $i < sizeof($planets); $i += 2) {
        $update = "UPDATE `u743148202_csis_2440`.`PLANETS` SET `Active` = 'Y' "
            . "WHERE (`PLANET_NAME` = '" . $planets[$i]['name'] . "')";
        ...
        if ($success == FALSE) {
            ...
            echo $planets[$i]['name'] . " was updated<br>";
        }
    }

    //search
    echo "<br><h2>This is a table of all the planets</h2><br><hr>";
    $search = "SELECT * FROM u743148202_csis_2440.PLANETS";

    $return = $con->query($search);

    if (!$return) {
        $failmess = "Whole query " . $search;
        echo $message;
        die('Invalid query: ' . mysqli_error($con));
    }
    echo "<table><th>Name</th><th width = '10%'>X,Y,Z</th><th>Description</th><th>Station</th>";
    while ($row = $return->fetch_assoc()) {
        echo "<tr><td>" . $row['PLANET_NAME']
            . "</td><td style='text-align:center;'>" . $row['COORD_X'] . ", " . $row['COORD_Y'] . ", " . $row['COORD_Z']
    ...
    //erase table data
    $truncate = "TRUNCATE table u743148202_csis_2440.PLANETS";
    $return = $con->query($truncate);
    //close DB connection
    mysqli_close($con);
    ?>
